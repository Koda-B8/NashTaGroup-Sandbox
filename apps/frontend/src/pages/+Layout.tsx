...
export default function MainLayout() {
	...
	function chooseCat(e: React.ChangeEvent<HTMLInputElement>, id: string): void {
		if (id) {
										...
										>
											...
											<ul className="flex flex-col gap-1 text-[15px] mt-2 w-full ml-0">
												{group?.options?.map((option) => (
													...
													>
														<input
															onChange={(e) => chooseCat(e, option.id)}
															type="radio"
															name={group.title}
															id={option.id}
															className="peer"
	...
	return (
		...
		>
			<div className="p-2">
				<Outlet context={{ categoryId, brandId }} />
			</div>
		</Shell>
	);
}
